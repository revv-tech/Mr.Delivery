...
// Agregar Arista
void Grafo::agregarArista(QString origen, QString destino, int peso,float km,float minutos){

    int orig = indexOfVertice(origen);
    int dest = indexOfVertice(destino);

    if (orig != -1 && dest != -1){
        matrizCosto[dest][orig] = peso;
        matrizDistancia[dest][orig] = km;
        matrizMinutos[dest][orig] = minutos;
        matrizStates[dest][orig] = true;
    }
   }
